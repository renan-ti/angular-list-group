		...
		.controller(
				...
				[
						...
						function($scope, $q, $timeout, $sce) {
							 ...
							 * 
							 */
							$scope.selection01 = $scope.selection02 = $scope.selection03 = null;
							...
							$scope.myDatasource = function() {
								var deferred = $q.defer();
								var promise = deferred.promise;
								$timeout(function() {
									deferred.resolve($scope.colors);
								}, 1000);
								return promise;
							}
							...
							$scope.beforeSelectionChangeAsyncHandler = function(
									item) {
								var deferred = $q.defer();
								var promise = deferred.promise;
								$timeout(function() {
							...
							$scope.afterSelectionChangeHandler = function(item) {
								$scope.message = "After selection change, "
										+ angular.toJson(item);
							}
							$scope.disabled = true;
