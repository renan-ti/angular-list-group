(function (window, document) {
  ...
  var ListGroupCtrl = [
      ...
      function ($scope, $attrs, $parse, $filter) {
        ...
        this.beforeSelectionChange = !$attrs.beforeSelectionChange ? defaultBeforeSelectionChange : $scope.beforeSelectionChange;
        ...
        this.select = function (item) {
          if (!this.isDisabled(item)) {
            var output = this.beforeSelectionChange({ item: item });
              ...
              if (angular.isFunction(output.then)) {
                output.then(function (returnedValue) {
                  if (returnedValue === true) {
                    ...
                    self.afterSelectionChange({ item: item });
                  }
                });
              } else if (output === true) {
                this.$selectItem(item);
                this.afterSelectionChange({ item: item });
              }
            }
          }
        };
        ...
        this.resolveLabel = function (item) {
          ...
          if ($attrs.labelFn) {
            ...
            label = fn($scope.$parent, { item: item });
          } else if (item.label) {
            label = item.label;
          }
          return label;
        ...
        var removeSelectedItemsListener = $scope.$watchCollection('listGroupCtrl.$$selectedItems', function (newValue, oldValue) {
  ...
  angular.module('listGroup').run([
    ...
    function ($templateCache) {
      ...
      $templateCache.put('linked-list-group.tpl.html', '<div class="list-group"><a ng-href="" class="list-group-item {{listGroupCtrl.resolveContextualClass(item)}}" ng-repeat="item in listGroupCtrl.$$items | filter:listGroupCtrl.filter.text:compare track by $index" ng-class="{active : (listGroupCtrl.isSelected(item) != -1), disabled : listGroupCtrl.isDisabled(item) }" ng-click="listGroupCtrl.select(item)">{{listGroupCtrl.resolveLabel(item)}}</a></div>');
      $templateCache.put('list-group.tpl.html', '<ul class="list-group"><li class="list-group-item {{listGroupCtrl.resolveContextualClass(item)}}" ng-repeat="item in listGroupCtrl.$$items | filter:listGroupCtrl.filter.text:compare track by $index">{{listGroupCtrl.resolveLabel(item)}}</li></ul>');
    }
  ]);
}(window, document));