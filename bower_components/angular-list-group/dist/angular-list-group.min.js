(function (window, document) {
  ...
  angular.module('listGroup').run([
    ...
    function ($templateCache) {
      ...
      $templateCache.put('linked-list-group.tpl.html', '<div class="list-group"><a ng-href="" class="list-group-item" ng-repeat="item in items track by $index" ng-class="{active : (isSelected(item) != -1), disabled : isDisabled(item) }" ng-click="select(item)">{{resolveLabel(item)}}</a></div>');
      $templateCache.put('list-group.tpl.html', '<ul class="list-group"><li class="list-group-item" ng-class="resolveItemClass(item)" data-ng-repeat="item in items track by $index">{{resolveLabel(item)}}</li></ul>');
