(function (window, document) {
  ...
  var ListGroupCtrl = [
      ...
      function ($scope, $attrs, $parse) {
        ...
        $scope.$selectedItems = [];
        $scope.beforeSelectionChange = !$attrs.beforeSelectionChange ? defaultBeforeSelectionChange : $scope.beforeSelectionChange;
        $scope.afterSelectionChange = !$attrs.afterSelectionChange ? noOp : $scope.afterSelectionChange;
        $scope.select = function (item) {
          if (!$scope.isDisabled(item)) {
            var output = $scope.beforeSelectionChange({ item: item });
              ...
              if (angular.isFunction(output.then)) {
                output.then(function (returnedValue) {
                  if (returnedValue === true) {
                    ...
                    $scope.afterSelectionChange({ item: item });
                  }
                });
              } else {
                $scope.$selectItem(item);
  ...
  angular.module('listGroup').run([
    ...
    function ($templateCache) {
      ...
      $templateCache.put('linked-list-group.tpl.html', '<div class="list-group"><a ng-href="" class="list-group-item {{resolveContextualClass(item)}}" ng-repeat="item in items track by $index" ng-class="{active : (isSelected(item) != -1), disabled : isDisabled(item) }" ng-click="select(item)">{{resolveLabel(item)}} {{contextualClass}}</a></div>');
      $templateCache.put('list-group.tpl.html', '<ul class="list-group"><li class="list-group-item {{resolveContextualClass(item)}}" data-ng-repeat="item in items track by $index">{{resolveLabel(item)}}</li></ul>');
